...
#---------------------------------------
class AnomalyModel:
	def __init__(self, trainingSet, anomalyMethod = "KNN" ):
...
################################################################################################
def getPValue_V1( all_buses, id_bus, i, alpha_m, anomalyMethod = "KNN" ):
	alphas = []
	for id_bus_j in range( len(all_buses) ):
		...
		model = AnomalyModel(flat_fleet_, anomalyMethod)
		for his_j in own_:
...
################################################################################################
def getPValue_V2( all_buses, id_bus, i, alpha_m, anomalyMethod = "KNN" ):
	alphas = []
	for id_bus_j in range( len(all_buses) ):
		if id_bus_j == id_bus: continue;
		...
		model = AnomalyModel(own_, anomalyMethod)
		for his_j in own_:
...
################################################################################################
def normalityProba_V1( method, flat_fleet_test_, his_test, all_buses, id_bus, i ):
	model = AnomalyModel(flat_fleet_test_, method)
	alpha_m = model.getAnomalyScore(his_test)
	pvalue = getPValue_V1( all_buses, id_bus, i, alpha_m, method )
	return pvalue, alpha_m

################################################################################################
def normalityProba_V2( method, flat_fleet_test_, his_test, all_buses, id_bus, i ):
	model = AnomalyModel(flat_fleet_test_, method)
	alpha_m = model.getAnomalyScore(his_test, inversed = True)
	pvalue = getPValue_V2( all_buses, id_bus, i, alpha_m, method )
	return pvalue, alpha_m
