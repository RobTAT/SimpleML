		...
		for ix, x in enumerate(self.Ux):
			...
			#---------------------------------------------------------
				...
				if ix in ids[:self.optimization_limit]:
					...
					# ---------------------
					# points_impacted = [xdp for xdp in self.Tx if temp_clf.predict_label(dp) != self.clf.predict_label(dp)]
					...
					for idp, dp in enumerate(self.Tx):
						...
						if temp_clf.predict_label(dp) != self.clf.predict_label(dp): diff.append( temp_clf.getPredictProba(1,dp) )
